...
export function FileUpload({
    ...
    React.useEffect(() => {
        ...
        currentFiles.forEach((file) => {
            // Detectar si es imagen o PDF para generar la URL de previsualización
            const isImage = file.type.startsWith("image/");
            const isPDF = file.type === "application/pdf" || file.name.toLowerCase().endsWith('.pdf');

            if ((isImage || isPDF) && !newPreviews[file.name]) {
        ...
        return () => {
            // Solo limpiar si no hay archivos (para evitar que desaparezcan las previews activas)
        ...
        <div className="min-w-55 w-full space-y-2.5">
            ...
            <div className="w-full space-y-2 max-h-75 overflow-y-auto custom-scrollbar">
                {currentFiles.map((file, index) => {
                    ...
                    const canPreview = (isImage || isPDF) && isDone;
                    ...
                    return (
                        ...
                        >
                            {/* Thumbnail interactiva para Preview */}
                            <div
                                className={cn(
                                    "relative h-10 w-10 md:h-12 md:w-12 shrink-0 rounded-md overflow-hidden border bg-muted flex items-center justify-center shadow-sm",
                                    canPreview && "cursor-pointer group/preview"
                                )}
                                onClick={() => {
                                    if (canPreview && previews[file.name]) {
                                        setPreviewModal({
                                            isOpen: true,
                                            url: previews[file.name],
                                            name: file.name
                                        });
                                    }
                                }}
                            >
                                {isImage && previews[file.name] ? (
                                    <>
                                        <img src={previews[file.name]} alt="Preview" className="h-full w-full object-cover transition-transform duration-300 group-hover/preview:scale-110" />
            ...
            {/* Renderizado del Modal de Previsualización */}
            <FilePreviewModal
